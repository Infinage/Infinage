...
#Copy plug.vim to $XDG_DATA_HOME/nvim/site/autoload/plug.vim
...
# Activate dev conda env
[[ "$MAMBA_DEFAULT_ENV" == "dev" ]] || mm activate dev
...
# TLDR config dir, doesnt auto pick since we broke xdg and appdata paths
...
mmsize() {
    ...
    case "$OSTYPE" in
      msys*|cygwin*|win32*) 
          conda_path=$(echo "$conda_path" | sed 's|\\|/|g; s|^\([A-Za-z]\):|/\L\1|') ;;
    esac

    if [ -z "$conda_path" ]; then
        echo "Error: CONDA_PREFIX is not set. Activate an environment first."
        return 1
    fi

    grep '"size":' "${conda_path}/conda-meta/"*.json 2>/dev/null \
      | sed -E 's|^.*/([^/]+)\.json:.*"size": ([0-9]+).*|\1\t\2|' \
      | sort -k2 -nr \
